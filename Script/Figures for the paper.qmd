...
ggsave(plot1, filename = "Plots/Fig1.png",width = 8.11, height = 12.98)
```

## One plot = one behaviour
...
```{r}
(plot.lineA <- plot_var_fish(data, "Swimming") +
   ylab("Time spent swimming (sec)")+
   ggtitle("A. Swimming"))
ggsave(plot.lineA, filename = "Plots/FigFishSwimming.png", width = 5.67, height = 6.37)

(plot.lineB <- plot_var_fish(data, "Resting") +
   ylab("Time spent resting (sec)")+
   ggtitle("B. Resting"))
ggsave(plot.lineB, filename = "Plots/FigFishResting.png", width = 5.67, height = 6.37)

(plot.lineC <- plot_var_fish(subset(data, Hovering >0), "Hovering") +
   ylab("Time spent hovering (sec)")+
   ggtitle("C. Hovering"))
ggsave(plot.lineC, filename = "Plots/FigFishHovering.png", width = 5.67, height = 6.37)
```
...
```{r}
(plotRPtank <- data_RP %>% group_by(Tank, Resting.place) %>% summarise(Resting = mean(Resting)) %>% 
...
```{r}
(plotITtank <- data_IT %>% group_by(Tank, IT) %>% summarise(Interation.with.surface = mean(Interation.with.surface)) %>% 
   ggplot(., aes(y = Interation.with.surface, x = Tank,fill = IT))+
   geom_bar(position = position_fill(reverse = TRUE), stat = "identity", color = "black")+
   scale_fill_manual(values=palette_IT)+
   theme(legend.title = element_blank())+
   ylab("% of interaction time")+
   scale_y_continuous(labels=c(0,25,50,75,100))+
   ggtitle("A"))

(plotITfish <- data_IT %>% group_by(Fish, IT) %>% summarise(Interation.with.surface = mean(Interation.with.surface)) %>% 
    ggplot(., aes(y = Interation.with.surface, x = Fish,fill = IT, width=.75))+
    geom_bar(position = position_fill(reverse = TRUE), stat = "identity", color = "black")+
    scale_fill_manual(values=palette_IT)+
    scale_y_continuous(labels=c(0,25,50,75,100)) +
    ylab("% of interaction time")+
...
(plotIT <- ggarrange(plotITtank,plotITfish, align = "h", widths = c(0.5, 1),
                    common.legend = TRUE, legend = "top", nrow = 1))
ggsave(plotIT, filename = "Plots/FigIT.png", width = 6.07, height = 4.22)
```
