...
```{r}
(plotRPtank <- data_RP %>% group_by(Tank, Resting.place) %>% summarise(Resting = mean(percentage)) %>% 
...
(plotRPfish <- data_RP %>% group_by(Fish, Resting.place) %>% summarise(Resting = mean(percentage)) %>% 
