...
## Calculating weights depending on residual variance
...
It worked: No more increasing variance with fitted values. But negative SumSq values for Tank, Wtf?  

If using higher weights, no negative value of SS but it is not fixing the problem of residuals. 

## Adjusted weight depending on group tank
...
Twice more variance for Jar. It sounds logical as it is one of the only group where fish perform often hovering. Creating the problem in residuals? Adjusting the weights depending on 
