...
rd <- rpt(Swimming ~ Tank + Time + Filter + (1|Fish), grname = "Fish", data = data, datatype = "Gaussian", nboot = 1000, npermut = 1000)
print(rd)
```
...
XXXXXXXXXXXX Repeatability and between-individual differences
...
plot_var_binary_fish(data, "SS.bin")
```
...
Even if there are less zeros than Foraging or Stereotypic Swimming, it is not a normal distribution. Even after removing the zeros, the distribution does not look like normal. Tring transformation

```{r}
ggplot(data, aes(x = log(Hovering + 1)))+
  geom_histogram(bins = 10, fill = grey(0.75), color="black")
ggplot(data, aes(x = log(Hovering + 4)))+
  geom_histogram(bins = 10, fill = grey(0.75), color="black")
ggplot(data, aes(x = log(Hovering + 7)))+
  geom_histogram(bins = 10, fill = grey(0.75), color="black")
