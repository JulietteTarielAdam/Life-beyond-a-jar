---
title: "Analysis of Naomi's data by Juliette. Influence of the tank on Betta's behaviour."
date: "`r Sys.Date()`"
format: 
   html:
...
Remember that we decided to compare Jar/Small/Medium/Large among each other, and Large/Barren between each other, because Jar/Small/Medium and Barren differ by two factors: enrichment and size. 

You fill find below the first row of tabs to click on (PCA, Swimming, etc) and for most of these tabs, a second row of tab to click on (Distribution, Plot, etc).

::: {.panel-tabset}

# Data description

9 behaviours scored: Resting, Swimming, Hovering, Sinking/Floating, Stereotypic swimming, Nest building, Foraging, Interation with surface, Out of view. Out.of.view is a bit special because it is not a behaviour. During a 10 min trial = 600 sec, the fish's behaviour was assigned to one of the 9 behaviours, meaning that the behaviours are mutually exclusive (a fish can't be scored as nest building and interacting with the surface at the same time).

After staying 3-7 days in a tank, the behaviour of all fish were scored by 4 trials in this tank. All trials occurred the same day at 4 different times (7 am, 10 am, 2 pm, 6 pm). 
...
I inspected the data for data entry errors (sum of scored behaviours too low/high, column shifted, typos, inconsistent nb of observations per fish/time/tank...) and Naomi corrected them. 
...
In my opinion, Filter needs to be included in all models as a control variable.

## Order of trials

Visual inspection of any trend in the order of trials, like for instance if there were more swimming during the 2nd trial whatever the tank was.
...
There was no apparent trend in the order of trials (for instance, fish did not perform more swimming at the 2nd trial whatever tank they were in).
...
The PCA did not help reduce the number of variables to analyse (see Analysis_PCA.html for more explanation) so I analysed the behavioural types separately + it allows to interpret differences between tanks quantitatively. I still plotted below the first 3 components, which explained 57% of variance in the data, to show the important variables and the correlations between variables. 
...
Sinking/Floating contributed almost nothing to the first three components (2%) + did not correlate strongly with any of the component. Sinking/Floating was not important to explain the variability between trials.
...
The second plot is to see the trend by fish and by tank. Thick red line is the overall mean by tank. Thin black lines are the mean for each fish by tank. 

```{r}
#| warning: false
plot_var(data, "Swimming")
plot_var_fish2(data, "Swimming")
```
...
+ Time. Fish swim less in the afternoon.
+ Filter. Fish swim more with a filter in their tank.

Repeatability is quite low (0.2)
...
```{r}
dataUpDown %>% filter(Swimming!=0) %>% 
ggplot(.,aes(y = Swimming, x = Tank, color = Up.Down)) +
...
+ Time. Fish rest more time in the afternoon. In line with the PCA and the results of Swimming.
...
Just a plot with the amount of time resting at the different places depending on the tank. Each bar value is the mean of resting times over trials. 

> Problem ->  Resting on plants in Barren and Jar??
...
tf(data_RP , grouping = c("Resting.place","Tank"), c("Resting","Swimming"), colMeans)  %>% 
    ggplot(., aes(fill = Resting.place, y = Resting, x = Resting.place))+
...
tf(data_RP , grouping = c("Resting.place","Fish"), c("Resting","Swimming"), colMeans)  %>% 
    ggplot(., aes(fill = Resting.place, y = Resting, x = Resting.place))+
    geom_bar(position = position_dodge(), stat="identity", color=grey(0.4))+
  scale_fill_manual(values = palette_RP)+
  facet_wrap("Fish")+
...
For the resting place, I don't really know what to do and in fact what would be interesting to analyse. Fish are already shifting their resting places between tanks just because they have access to new resting places (filter, plants).

It might be just interesting to say that fish use different places to rest, not only one type. It is particularly visible in Large where fish use all types of resting places. 
...
The data are clearly not following a normal distribution. It seems that they could possibly follow a beta distribution but I thought beta distributions were when the variable is between 0 and 1. I decided therefore to analyse Foraging in binary: 1 if fish spent time foraging during this trial, 0 otherwise.
...
plot_var_binary(data, "Foraging.bin") + facet_wrap(~Fish)
```
...
```{r}
#| warning: false
plot_var_binary(data, "SS.bin")
plot_var_binary(data, "SS.bin") + facet_wrap(~Fish)
```
...
data_SST %>% filter(Stereotypic.swimming.type!= "Group 1",
                Stereotypic.swimming.type!= "Group 2") %>% 
ggplot(., aes(fill = Stereotypic.swimming.type, x = Tank))+
    geom_bar(stat= "count",color=grey(0.4))+
    theme(legend.position="bottom")+
    ylab("Count of trials")
```

Problem in the data: what is "Group1" and "Group2"? Check the data. 

Similarly with the types of Interaction with Surface, I suggest grouping different types of stereotypic swimming together to make it clearer.
...
data_SST2 %>%
  ...
  arrange(Tank, Fish, Time)
```

I don't think there are enough repetitions to really say something about the number of repetitions. The number of repetitions seems pretty unique to a fish in a certain tank. I don't see a pattern depending on the tank. The interesting thing is that the stereotypic swimming seemed consistent in a tank for a fish (most or all trials of that day with stereotypic swimming). I don't know if it is possible to calculate the repeatability of a binomial glm, but it would be interesting. 
...
Even if there are less zeros than Foraging or Stereotypic Swimming, it is not a normal distribution. Even after removing the zeros, the distribution does not look like normal. Unless having a better idea, Hovering will be analysed as a binary variable. 
...
```{r}
#| warning: false
plot_var_binary(data, "Interacting.bin")
plot_var_binary(data, "Interacting.bin") + facet_wrap(~Fish)
```
...
+ Tank. Fish were more likely to interact with the surface during a trial in Jar/Medium compared to Large (not significant for Small). Fish in Jar were XX times [CI XX, XX] more likely to interact with the surface during a trial than fish in Large, and Fish in Medium were XX times [CI XX, XX] then fish in Large (do the same calculation than Foraging or Stereotypic Swimming). In addition, fish were XX times [CI XX, XX] more likely to interact with a surface during a trial in Barren than in Large. 
...
I have analysed Sinking/Floating, but if it were up to me, I would give up this variable in view of the PCA. 
