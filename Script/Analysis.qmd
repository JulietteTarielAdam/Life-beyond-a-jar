...
```{r}
summary(data_RP2 %>% dplyr::select(Floor, Furnishings, Water.surface))
# number of zeros (no resting at this place during the trial)
data_RP2 %>% dplyr::select(Floor, Furnishings, `Water surface`) %>% 
...
```{r}
library(brms)
m1 <- brm(cbind(Floor, Water.surface, Furnishings) ~ Tank + Order + Time + Filter + (1|Fish), data = data_RP2, family = categorical(link = "logit"))
