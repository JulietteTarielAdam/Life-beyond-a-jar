...
Linear model with weighted least squares. See details for the weights'calculation on Analysis_Hovering.

```{r}
# temporary model to calculate weights to handle increasing error variance with fitted values
mHovering2_temp <- lmer(Hovering ~ Tank + Time + Filter + Order + (1 | Fish), data = data[data$Hovering>0,])
weights_hovering <- data.frame(abs.res = abs(residuals(mHovering2_temp )),
                               fit.values = fitted(mHovering2_temp))
weights_hovering$var <- predict(lm(abs.res ~ fit.values + I(fit.values^2), data = weights_hovering))^2
weights_hovering$weights <- 1 / weights_hovering$var

# Final model 
mHovering2 <- lmer(Hovering ~ Tank + Time + Filter + Order + (1 | Fish), data = data[data$Hovering>0,],
                  weights = weights_hovering$weights)
anova(mHovering2, type = 2)
