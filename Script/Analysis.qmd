...
The script contains the code of data transformation, analysis and graphs for the article "XXXXXX". It is in two versions: the raw R script in qmd format and the code output in html format. 

The goal was to test the effect of Tank on Siamese fish behaviour. The tanks Jar, Small, Medium and Large are compared among each other, and the tank Large and Barren between each other. Barren is not compared to Jar, Small or Medium because they differ by two factors: enrichment and size. 

On the html version of the scode, you fill find below a first row of tabs to click on (PCA, Swimming, etc) and for most of these tabs, a second row of tabs to click on (Distribution, Plot, etc).
...
data.full$total <- rowSums(dplyr::select(data.full, beh.cols.full)) 
sort(data.full$total) 
```
...
plot1("Tank", main = data.full, beh_cols = beh.cols.full, palette = palette.beh)

plot1("Tank","Fish", palette = palette.beh) +  facet_wrap_paginate(~Fish,ncol=2,nrow=2, page = 1) + ggtitle("Per fish") 
plot1("Tank","Fish", palette = palette.beh) +  facet_wrap_paginate(~Fish,ncol=2,nrow=2, page = 2) + ggtitle("Per fish")
plot1("Tank","Fish", palette = palette.beh) +  facet_wrap_paginate(~Fish,ncol=2,nrow=2, page = 3) + ggtitle("Per fish")
plot1("Tank","Fish", palette = palette.beh) +  facet_wrap_paginate(~Fish,ncol=2,nrow=2, page = 4) + ggtitle("Per fish")

plot1("Time") + ggtitle("Per time")
```
...
Fish underwent different order of treatments: Some fish started with Large, then Small, etc; Other fish with Medium, then Jar, etc. Is there an effect of the order of treatments? For instance, were the fish more swimming during the   on average the same amount of time in Large when Large was the 2nd or 3rd tank they were in? Visual investigation using the order of tanks. Barren was the last tank for all fish. 
...
The PCA did not help reduce the number of variables to analyse (see Analysis_PCA.html for more explanation) so the behaviours were analysed separately with linear models. The advantage compared to the PCA is the possibility to quantitatively interpret the differences between tanks.
...
(plotPCA2 <- plot(pca, choix = "var", axes = c(3,4)) + labs(title = NULL))
```
...
We fitted a glm distribution like Foraging and Stereotypic Swimming on the probability to hover (dependent variable is 1 if hovering occured during the trial, zero otherwise).
...
The analysis output of all these models are in the script Analysis Hovering.

Here after only the results of the glm on the probability to hover and the results of the lmm on the amount of time spent hovering with weighted least squares (for trials with hovering time > 0).
...
The model seems good besides the normality. But as said earlier, linear models are robust to the normality assumption (= we can still draw true inference).
...
(plotRP1 <- ggplot(data_HP, aes(y = Hovering, x = Hovering.place, fill = Hovering.place)) +
  stat_summary(fun = mean,geom = "bar", size = 4)+
  geom_jitter(alpha = .5, width = 0.05, height=0, color = grey(0.25)) +
  scale_color_manual(values=palette_HP)+
  facet_wrap(~Tank)+
  guides(fill = FALSE)+
...
Error when calculating the repeatability with rptR, item Foraging. 
...
```{r}
mInteracting0 <- glm(Interacting.bin ~ Tank + Time + Filter, data = data, family =binomial)
lrtest(mInteracting0, mInteracting)
```
...
```{r}
mNest0 <- glm(Nest.bin ~ Tank + Time + Filter, data = data, family =binomial)
lrtest(mNest0, mNest)
```
...
```{r}
# Tank
beh.cols2 <- beh.cols[-which(beh.cols=="Sinking.Floating")] # remove Sinking.Floating

(plot.tank <- plot1("Tank", beh_cols = beh.cols2, palette = palette.beh2) + 
  ylab("Time spent doing the behaviour (sec)")+
  theme(legend.position="right") +
  guides(fill=guide_legend(title="Behaviour")))

(plot.time <- plot1("Time", beh_cols = beh.cols2, palette = palette.beh2) + 
  ylab("Time spent doing the behaviour (sec)")+
  theme(legend.position="right") +
  guides(fill=guide_legend(title="Behaviour")))

(plot.filter <- plot1("Filter", main = data, beh_cols = beh.cols2, palette = palette.beh2) + 
  ylab("Time spent doing the behaviour (sec)")+
  theme(legend.position="right") +
  guides(fill=guide_legend(title="Behaviour")))
```
...
## Time spent
Resting, hovering, swimming
```{r}
(plotSwimming <- plot_var(data, "Swimming") + ylab("Time spent swimming (sec)"))

```

Plot each fish time with individual data? 
plotRP1
plotRP2
