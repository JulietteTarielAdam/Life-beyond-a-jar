...
## Plot
```{r}
ggplot(dataUpDown, aes(y = perc.up, x = Tank)) +
    geom_jitter(alpha = .5, width = 0.05, height=0, color = grey(0.25)) +
...
## Model
For percentages, we can fit a beta distribution. All percentages need to be 0 < perc < 100%. There are trials with 100% so we can't use beta distribution. 

We can fit a binomial distribution. The percentages need to be counts. It works with counts of seconds (how many seconds the fish spend up of the tank), just needed to round the counts of seconds.


```{r}
mUp <- glmmTMB(cbind(up, down) ~ Tank + Time + Filter + (1|Fish), data = dataUpDown, family = binomial)
mUp_res <- simulateResiduals(mUp)
plot(mUp_res)
Anova(mUp)

mUp <- lm(cbind(up, down) ~ Tank + Time + Filter + (1|Fish), data = dataUpDown, family = binomial)

calc_contrasts_bin(mUp)


mUp2 <- lmer(up ~ Tank + Time + Filter + (1|Fish), data = dataUpDown)
lm_diagnosis(mUp2)

```

Rarely seen such bad diagnosis plots. 
...
mUp_res <- simulateResiduals(mUp)
plot(mUp_res)
plotResiduals(mUp_res, form = dataUpDown$Tank)
plotResiduals(mSS_res, form = data$Time)
plotResiduals(mSS_res, form = data$Filter)
# Problem with Filter, I retried the model without Filter
mSS2 <- glmmTMB(SS.bin ~ Tank + Time + (1|Fish), data = data, family =binomial)
