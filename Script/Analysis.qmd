...
Problem is that the total resting time is different for each trial.We had to analyse the percentage of time resting at the different places rather than the raw time.
...
A lot of trials where the fish did not rest at all on Filter, Surface or Barrel. This is going to create a problem for the analysis (zero inflation).
...
emmeans(modelRP1, pairwise ~ Resting.place, type = "response")$contrasts
```
...
zero_infl_model <- zeroinfl(percentage ~  Tank + Time + Filter + Order + Resting.place, data = data_RP, dist = "binomial")
