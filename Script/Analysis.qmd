...
```{r}
#| include: false
# This code allows to run the scripts that load the data and that contains the functions to plot and do the linear models
...
Out.of.view is a bit special because it is not a behaviour and was not analyse.

Sinking/Floating was not analyse (see explanation in the conclusion of the PCA section).
...
```{r}
(plot1 <- plot(pca, choix = "var") + labs(title = NULL))
(plot2 <- plot(pca, choix = "var", axes = c(1,3)) + labs(title = NULL))
# Export plots
arrange <- ggarrange(plot1, plot2, labels =LETTERS[1:2])
...
```{r}
#| warning: false
plot_var(data, "Swimming") + ylab("Time spent swimming (sec)")
...
save(rpt.swimming, file = "Scripts/Repeatability rptR/rpt.swimming")
```
