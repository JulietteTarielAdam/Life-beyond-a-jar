...
The total resting time is different for each trial -> we can't use directly the raw resting time at each place.

Trying to use a multinomial regression model: extension of the binomial logit model to more than two categories. 

Problem: Barrel and Filter as not possible categories for all fish. Solution is to concatenate Barrel, Filter and Plants as one category furnishings. 
...
#| echo: false
#| message: false
data_RP %>% group_by(Resting.place) %>%  
  summarise(mean = mean(percentage)*100, 
            N = n(), 
            count_Zeros = sum(Resting == 0),
            perc_zero = count_Zeros*100/N)

data_RPwz %>% group_by(Resting.place) %>%  
  summarise(mean_without_zeros = mean(percentage)*100, 
            N_without_zeros = n())

# plot excluded Filter otherwise we don't see the distribution for the other resting places
data_RP %>% filter(Resting.place != "Filter") %>% 
ggplot(., aes(percentage, fill= Resting.place)) + geom_density(alpha = 0.5)
```

A lot of trials where the fish did not rest at all on Filter and Surface. This might create a problem for the analysis (zero inflation).
...
library(car)
library(DHARMa)
modelRP1 <- glmmTMB(binary~  Resting.place + Tank + Time + Filter + Order + (1|Fish), data = data_RP, family = binomial)
Anova(modelRP1)
modelRP1_res <- simulateResiduals(modelRP1)
plot(modelRP1_res)
plotResiduals(modelRP1_res, form =  data_RP$Resting.place)

emmeans(modelRP1, pairwise ~ Resting.place, type = "response")
```

Barrel, Surface and Filter have a lower probability of being used as a resting place during a trial than Floor or Plant.
No significant difference between Barrel, Surface or Filter. No significant difference between Floor or Plant. 

## Analysis without zero

I also remove Filter as a whole as there is only one trial where a fish rested on the filter. 

```{r}
ggplot(data_RPwz, aes(percentage, fill= Resting.place)) + geom_density(alpha = 0.5)

modelRP2 <- glmmTMB(percentage~  Resting.place + Tank + Time + Filter + Order + (1|Fish), data = data_RPwz, family = binomial)
...
tableRP <- data_RP %>% group_by(Resting.place) %>% summarise(Mean.resting = mean(Resting))
tableRP$percentage <- tableRP$Mean.resting *100/ sum(tableRP$Mean.resting)
tableRP
ggplot(data_RP, aes(y = Resting, x = Resting.place))+
  geom_boxplot()
  
data_RP$Resting.place2 <- ifelse(data_RP$Resting.place %in% c("Plant", "Barrel", "Filter"), "Combined", as.character(data_RP$Resting.place))
tableRP2 <- data_RP %>% group_by(Resting.place2) %>% summarise(Mean.resting = mean(Resting))
tableRP2$percentage <- tableRP2$Mean.resting  *100/ sum(tableRP2$Mean.resting )
tableRP2
ggplot(data_RP, aes(y = Resting, x = Resting.place2))+
  geom_boxplot()
```
